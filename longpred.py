...
def get_pred(rank, world_size, data, max_length, max_gen, prompt_format, dataset, device, model_name, model2path, out_path):
    ...
    for json_obj in tqdm(data):
        ...
        out_folder = f"predlong/{model_name}"
        if not os.path.exists(out_folder):
            os.makedirs(out_folder)
        out_path = os.path.join(out_folder, f"{dataset}.jsonl")
...
def load_model_and_tokenizer(path, model_name, device):
    ...
    elif "llama2" in model_name or "Llama" in model_name:
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        if args.e:
            ...
            if not os.path.exists(f"pred_elong/{model_name}"):
                os.makedirs(f"pred_elong/{model_name}")
            out_path = f"pred_elong/{model_name}/{dataset}.jsonl"
        else:
            data = load_dataset('THUDM/LongBench', dataset, split='test')
            if not os.path.exists(f"predlong/{model_name}"):
                os.makedirs(f"predlong/{model_name}")
            out_path = f"predlong/{model_name}/{dataset}.jsonl"
